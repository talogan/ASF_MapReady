...
main(int argc, char *argv[])
{
  ...
  sprintf(cmd,"focus rd %s.dat %s.ldr rd_full.gli rd_full.gli.par -pro SGF",raw_file, raw_file);
  err = system(cmd);

  sprintf(cmd,"focus cc -i rd_full.gli -p rd_full.gli.par -his %s.dat.his -pro SGF -dat rd_full.dat -lea rd_full.ldr -tra rd_full.tra -vol rd_full.vol -nul rd_full.null -log rd_full.log", raw_file);
  err = system(cmd);
  ...
  for (i=first_frame+1; i<last_frame; i++)
   { 
     ...
     sprintf(filename,"R1_%.5i_F.%.3i",meta->general->orbit,i);

     sprintf(cmd,"focus rd %s.dat %s.ldr %s.gli %s.gli.par -center_option LAT -center %lf -duration_option DIS -duration 50 -pro SGF",
		raw_file, raw_file, filename, filename, ers_frame[i]);
     err = system(cmd);

     sprintf(cmd,"focus cc -i %s.gli -p %s.gli.par -his %s.dat.his -pro SGF -dat %s.D -lea %s.L -tra %s.tra -vol %s.vol -nul %s.nul -log %s.log",filename,filename,raw_file,filename,filename,filename,filename,filename,filename);
     err = system(cmd);
